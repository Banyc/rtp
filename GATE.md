...
## The opening handshake's time budget

`OPENING_LEG_TIMEOUT` bounds **one** handshake leg, not the opening. The
client crosses two legs (`Hello`→`HelloAck`, then `Confirm`→`ConfirmAck`) and
the server one (`HelloAck`→`Confirm`); each is armed with its own budget at
the instant it starts, so a slow leg cannot spend the next leg's share. A leg
is one round trip on the path, so the budget is derived from the path's own
worst round trip: the operator's client measures a 190 ms minimum with maxima
of 1063 ms and 3205 ms, and 4 s is the smallest whole second above 3205 ms.
The whole opening is therefore bounded by two legs, and a path that never
answers costs one leg, because a leg that never answers never reaches the
second.

It cannot be derived from the handshake's own RTT sample: the opening is
lockstep, so both peers bound the *same* leg by a deadline armed before that
leg's sample exists, and a one-sided budget longer than the peer's would only
retry into an abandoned handshake.

Measured on a real `rtp`-over-`NetemPair` sweep: a throwaway integration
target that binds an `rtp` listener, runs the server opening inside
`accept_with`, and dials it with `connect_with { handshake: true }` through
`NetemPair::spawn(server, latency(owd), latency(owd))` — one-way delay per
direction, so the round trip is twice the first column, and `ok` is the
client's `connect_with` returning:

| one-way | round trip | before (3 s, whole opening) | after (4 s per leg) |
| --- | --- | --- | --- |
| 25 ms | 50 ms | ok 114 ms | ok 130 ms |
| 100 ms | 200 ms | ok 422 ms | ok 408 ms |
| 190 ms | 380 ms | ok 799 ms | ok 794 ms |
| 500 ms | 1000 ms | ok 2034 ms | ok 2020 ms |
| 1000 ms | 2000 ms | **fail 3037 ms** | ok 4020 ms |
| 1500 ms | 3000 ms | fail 3028 ms | ok 6005 ms |
| **1602 ms** | **3204 ms — the field's spike** | **fail 3018 ms** | **ok 6448 ms** |
| 1900 ms | 3800 ms | fail 3009 ms | ok 7628 ms |
| 2000 ms | 4000 ms | fail 3006 ms | **fail 8039 ms** |

The old shape failed on every round trip above 1500 ms (ok at 1500 ms round
trip / 3039 ms elapsed, fail at 1600 ms / 3032 ms in the bracketing reps) and
always with `io::ErrorKind::TimedOut`. The new one fails only once a leg
reaches its own 4 s budget. A path that never answers costs 4055 / 4017 /
4043 ms after the change (one leg, black-holed `connect_with`) against ~3030 ms
before: that increase is inherent, because a 3.2 s-spike path and a dead path
are indistinguishable until the answer arrives, so the budget that covers the
spike is the budget a dead path pays.

What a client does when an opening fails belongs to `rtp_mux`, and it is why
the old budget could not be rescued by retrying. The `rtp_mux` dual-lane birth
re-dials `MAX_DUAL_CONNECT_ATTEMPTS = 3` times with a 25 / 50 ms backoff
(`rtp_mux/src/connector/dial.rs`, `rtp_mux/src/shared.rs`), each attempt a
fresh opening with a fresh nonce and a fresh deadline. Three attempts at the
old 3 s budget is 9 s of trying — longer than the 3.205 s spike — but no single attempt outlasts
one round trip of a 3205 ms path, and the opening needs two, so all three fail
and the client sees roughly 9 s of failure and then `TimedOut` on a path that
was merely slow. Per-leg budgets make the first attempt succeed at ~6.4 s
instead.

Both pins go red when the behaviour is reverted. With the leg budget back at
3 s, `an_opening_leg_outlasts_the_fields_worst_round_trip` fails on its
relation (`its budget (3s) must outlast the field's worst measured round trip
(3.205s)`) and `the_opening_completes_at_the_fields_worst_round_trip` fails
with `Kind(TimedOut)`. With the value left at 4 s but the two client legs made
to share one deadline again — the shape the budget had before this change —
...
The tier that runs on every `cargo test` is `cargo test -p rtp`: the 732-test
...
| --- | --- | --- |
| `lib` | 721 (+11 ignored) | 6.99 / 10.27 / 7.00 s |
...
| **`cargo test -p rtp` end to end** | **736** | **15.88 / 15.53 / 15.44 s** |
...
assertions require. A paused runtime clock does not reach its stalls: with
`start_paused` a 300 ms `tokio::time::sleep` costs 0 ms of wall clock while the
same runs' repair stall still measures 299-301 ms, the runtime's clock tracking
that wait (328 ms of advance) rather than jumping to the deadline, so the row
keeps its wall clock under a paused test clock; shortening it would take
replacing the link and the repair timers the row exists to exercise. The two
session-bounds rows no longer pay their waits as wall clock at
all: their `DRIVER_JOIN_TIMEOUT` waits are driven by the same paused clock and
pinned at 0.002-0.007 s, so the `--lib` target's serialized per-test sum falls
by the 6.00 s they spent — three fresh reps measure 23.00/24.52/26.32 s of
per-test sum against 23.04/24.66/26.34 s of wall clock (load average 5.0-6.4 on
10 cores, of which the FEC row drew 1.61/4.73/9.03 s), three parallel reps
measure 3.97/6.14/6.35 s (load average 3.8-5.3), and three end-to-end reps of
`cargo test -p rtp` measure 16.72-18.11 s real against 16.13-17.59 s of
`finished in` figures over 14 binaries.
...
2.80-9.03 s contended), then the two decoder fuzzes at 1.84 s and 1.85 s,
then
...
entire 719-test lib target, which measured 8.12 s in debug on that session's
checkout**. The two rows are real
