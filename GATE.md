...
- `python3 tools/check-clock-seam.py` fails on any direct wall-clock read
  (`Instant::now`/`SystemTime::now`/`.elapsed()`) reintroduced into the opening
  handshake's decision code. That code must read the `clock::ClockRef` seam
  (`src/clock.rs`) so a deadline can be driven by a test runtime rather than
  slept through by wall time; without the guard the multi-second cost the seam
  removed comes back silently. The same checker also holds the rows whose wait
  was driven instead of slept through — the field-spike opening row must keep
...
| --- | --- | --- |
| `lib` | 721 (+11 ignored) | 5.12 / 9.47 / 8.69 s |
...
| **`cargo test -p rtp` end to end** | **736** | **19.78 / 12.01 / 11.96 s** |
...
2.57-2.60 s (±1 %), while the `lib` target spans 2.69-7.20 s —
...
assertions require. A paused runtime clock does not reach its stalls, re-tested
on this revision: with the row forced onto `#[tokio::test(start_paused =
true)]` and the attribute verified compiled in, five reps cost
1.78/1.98/2.05/2.39/3.40 s against 2.40/2.54/3.37 s on its usual multi-thread
runtime, no collapse toward the 0.17-0.24 s zero-loss floor; a directed probe
records a 300 ms `tokio::time::sleep` costing 0 ms of wall clock under pause
while the same runs' repair stall still measures 299-301 ms, the runtime's
clock tracking that wait (328 ms of advance) rather than jumping to the
deadline. The reason is the seam's size: the stall is the send driver parked in
`sleep_until` (`socket/session.rs`) on the wake `next_send_wake` computed from
the send pass's `Instant::now()` (`transmission/write_half.rs`), and the RTO it
parks on is armed in `connection.rs::send_bytes` from receive-path samples —
22 production direct-clock sites across `write_half.rs` (10), `connection.rs`
(10) and `socket/session.rs` (2) that share one time base, so a clock seam here
is a whole-engine migration rather than a module one, and a partial one mixes
bases (an RTO in the passed instant's past panics in `duration_since`).
Shortening it would also take replacing the link and the repair timers the row
exists to exercise: the row's point is that parity crosses loopback before an
RTO could deliver the lost symbol, so reordering the two by driving the clock
could satisfy `recovered > 0` through ARQ instead of FEC. The row keeps its
wall clock under a test clock, and the always-run tier's cost is what its
assertions require. The two
...
pinned at 0.002-0.007 s, and the field-spike opening row below is driven the
same way, so the `--lib` target's serialized per-test sum falls by the
6.00 s the session rows spent and the ~6.4 s the opening row spent. The removal
is not separable from this target's own noise: three fresh serialized reps
measure 19.19/24.94/24.75 s of per-test sum against 19.20/24.95/24.77 s of wall
clock (load average 2.3-2.6 on 10 cores), and the FEC row's own draw in
the same reps spans 2.38-9.16 s — a spread as large as the work removed, so
earlier reps of the same code span 19.70-27.03 s of sum. The fit check holds
exactly in every rep. Fresh parallel reps measure 2.69-9.47 s (load average
2.0-3.2), and three end-to-end reps of `cargo test -p rtp` measure
19.78/12.01/11.96 s real against 18.61/11.72/11.66 s of `finished in` figures
over 14 binaries.
...
per-test cost, the whole `--lib` target sums to 19.2-27.6 s measured the same
way, and the tier's wall clock is 11.7-19.8 s at its own parallelism — three
