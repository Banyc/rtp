...
| --- | --- | --- |
| `lib` | 721 (+11 ignored) | 2.69 / 7.20 / 5.66 s |
...
| **`cargo test -p rtp` end to end** | **736** | **19.99 / 11.65 / 11.59 s** |
...
measure 27.03/22.98/26.57 s of per-test sum against 27.04/22.99/26.59 s of wall
clock (load average 2.6-3.1 on 10 cores), and three more measure
19.70/22.27/25.95 s, because the FEC row's own draw in those reps is
2.82/5.10/9.09 s — a spread as large as the work removed. The fit check holds
exactly in every rep. Three parallel reps measure 2.69/7.20/5.66 s (load
average 2.6-3.2), and three end-to-end reps of `cargo test -p rtp` measure
19.99/11.65/11.59 s real against 19.67/11.37/11.29 s of `finished in` figures
over 14 binaries.
...
per-test cost, the whole `--lib` target sums to 19.7-27.0 s measured the same
way, and the tier's wall clock is 11.3-19.7 s at its own parallelism — three
