//! The observation sink: one connection's metrics callback, CSV logger, and
//! monotonic event index, fed by the typed schema in [`super::schema`].
...
/// an await.
#[derive(Debug)]
pub(crate) struct ConnectionObservability {
    clock_epoch: Instant,
...
impl ConnectionObservability {
    ...
    pub(crate) fn elapsed_since(&self, now: Instant) -> std::time::Duration {
    ...
    pub(crate) fn wants_snapshot(&self, event: MetricsEvent, elapsed: std::time::Duration) -> bool {
    ...
    pub(crate) fn next_event_index(&self) -> u64 {
